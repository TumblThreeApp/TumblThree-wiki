...
* Blog -> Settings applied to each blog upon addition:
  ...
  * Note: You might want to always select:
    ...
    *  _Force Rescan_: Force Rescan always checks the whole blog and not just new posts which were added after the last successful download. The statistics of a blog (total posts, number of post, number of duplicates) currently can only be updated if the whole blog is downloaded. Thus, disabling this might result in downloading "more" posts than displayed in TumblThree. If you don't care about the displayed blog statistics, turning Force Rescan off will decrease the scanning time since already downloaded posts are skipped in the scanning.
