...
Be aware that the filename template should contain a uniqueness else files will be missing in the downloads. Where as %f is the most unique resp. the old file naming style. But a combination out of %d, %u, %i or %k plus %n is also good.
...
If the image index token ("%n") is used, but the media isn't part of a photo set, then the token is ignored and a character repetition is removed (eg. "%i_%n_%f" -> "%i_%f").
